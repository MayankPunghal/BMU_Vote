...
class Vote extends Component {
  ...
  vote = async () => {
    let candidate = await this.state.VotingInstance.methods.candidateDetails(this.state.candidateId).call();
    // await this.state.VotingInstance.methods.addCandidate(this.state.name, this.state.party, this.state.manifesto, this.state.constituency).send({from : this.state.account , gas: 1000000});

    if(this.state.myAccount.constituency !== candidate.constituency){
      this.setState({toggle : true});
    }else{
      await this.state.VotingInstance.methods.vote(this.state.candidateId).send({from : this.state.account , gas: 1000000});
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      let myAccount = await this.state.VotingInstance.methods.voterDetails(this.state.account).call();
      this.setState({myAccount : myAccount});


      let candidateCount = await this.state.VotingInstance.methods.getCandidateNumber().call();

      let candidateList = [];
      for(let i=0;i<candidateCount;i++){
        let candidate = await this.state.VotingInstance.methods.candidateDetails(i).call();
