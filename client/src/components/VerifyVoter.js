...
class VerifyVoter extends Component {
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      for(let i=0;i<voterCount;i++){
          ...
          let voterDetails = await this.state.VotingInstance.methods.voterDetails(voterAddress).call();
  ...
  verifyVoter = async event => {
    await this.state.VotingInstance.methods.verifyVoter(event.target.value).send({from : this.state.account , gas: 1000000});
